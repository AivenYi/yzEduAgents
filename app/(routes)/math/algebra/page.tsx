...
export default function AlgebraPage() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem('algebraMessages');
    if (savedMessages) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (botMessage) {
        ...
        localStorage.setItem('algebraMessages', JSON.stringify(updatedMessages));
      }
    } catch (error) {
      console.error('API Error:', error);
      const errorMessages = [...messages, { role: 'bot', content: '抱歉，我暂时无法回答这个问题。' }];
      setMessages(errorMessages);
      localStorage.setItem('algebraMessages', JSON.stringify(errorMessages));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearHistory = () => {
    if (window.confirm('确定要清空所有聊天记录吗？')) {
      setMessages([]);
      localStorage.removeItem('algebraMessages');
    }
  };
    ...
    <MathLayout>
      <div className="split-container p-10">
        <div className="left-panel">
          <h2 className="text-white text-xl font-bold mb-6">代数</h2>
