...
import EnglishLayout from '../_components/english-layout';
...
export default function VocabularyPage() {
  ...
  useEffect(() => {
    const savedMessages = localStorage.getItem('vocabularyMessages');
    if (savedMessages) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (botMessage) {
        ...
        localStorage.setItem('vocabularyMessages', JSON.stringify(updatedMessages));
      }
    } catch (error) {
      console.error('API Error:', error);
      const errorMessages = [...messages, { role: 'bot', content: '抱歉，我暂时无法回答这个问题。' }];
      setMessages(errorMessages);
      localStorage.setItem('vocabularyMessages', JSON.stringify(errorMessages));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearHistory = () => {
    if (window.confirm('确定要清空所有聊天记录吗？')) {
      setMessages([]);
      localStorage.removeItem('vocabularyMessages');
    }
  };
    ...
    <EnglishLayout>
      <div className="split-container p-10">
        <div className="left-panel">
          <h2 className="text-white text-xl font-bold mb-6">词汇学习</h2>
        ...
        <div className="right-panel">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-white text-xl font-bold">学习结果</h2>
            <button
